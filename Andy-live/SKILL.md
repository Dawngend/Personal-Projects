...
* **LLM Orchestration & RAG:** Groq Cloud LPU (`openai/gpt-oss-20b`, `openai/gpt-oss-120b`, `qwen/qwen3.6-27b`), SentenceTransformers, ChromaDB
...
* **Key Components:**
  ...
  * **Groq Router (`smartRouter.js`):** Classifies query intent in ~5ms and routes to Groq models for talking points, reasoning, architecture, coding, and vision. Fresh-information requests disclose that live web grounding is unavailable.
  ...
  * **Session History:** Keeps question/response, provider, latency, role mode, and selected screenshot filename in memory for the active app session; users can clear, copy Markdown, or explicitly export Markdown.
  * **Debug Prompt Handoff:** On a runtime/provider failure, users can ask Groq to create and copy a sanitized troubleshooting prompt for Codex or Claude Code.
