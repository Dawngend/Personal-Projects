...
export function MathPrompt({ children }: { children: string }) {
  ...
  return <div className="math-prompt">{pieces.map((piece, index) => {
    ...
    return <span key={index} className={display ? "math-display" : "math-inline"} dangerouslySetInnerHTML={{ __html: katex.renderToString(piece.slice(2, -2), { displayMode: display, throwOnError: false, strict: "ignore" }) }} />;
  })}</div>;
}
