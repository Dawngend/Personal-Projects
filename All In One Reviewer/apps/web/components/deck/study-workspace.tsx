  ...
  return (
    <main className={styles.workspace}>
      ...
      <section className={styles.intro} aria-labelledby="workspace-title">
        ...
        <p>
          Choose a reviewer, or plant a new set of source materials for Andy to turn into active
          recall.
        </p>
      </section>
      <section className={styles.continue} aria-labelledby="continue-title">
        <div>
          <p className={styles.kicker}>Current thread</p>
          <h2 id="continue-title">No session is in progress</h2>
          <p>Your deck library stays here; focused quiz sessions arrive in the next study phase.</p>
        </div>
        <Link href="/decks/new" className={styles.primary}>
          Create a deck <span aria-hidden="true">→</span>
        </Link>
      </section>
      <section className={styles.library} aria-labelledby="library-title">
        <div className={styles.libraryHead}>
          <div>
            ...
            <h2 id="library-title">Study material, grouped by subject</h2>
          </div>
          <span className={styles.mono}>{decks.data?.length ?? 0} decks</span>
        </div>
        {decks.isLoading && <p className={styles.state}>Reading the deck library…</p>}
        {decks.isError && (
          <p className={styles.error}>
            The API could not load your decks. Check that the FastAPI service is reachable.
          </p>
        )}
        {!decks.isLoading && !decks.isError && decks.data?.length === 0 && (
          <div className={styles.empty}>
            ...
            <h3>No decks yet</h3>
            <p>
              Upload PDFs or slide decks, choose a question style, and generate your first reviewer.
            </p>
            <Link href="/decks/new">Start with materials</Link>
          </div>
        )}
        {Object.entries(grouped).map(([subject, subjectDecks]) => (
          <div className={styles.subject} key={subject}>
            <h3>{subject}</h3>
            <div className={styles.deckGrid}>
              {subjectDecks?.map((deck) => (
                <article className={styles.deck} key={deck.id}>
                  <p className={styles.mono}>DECK {String(deck.id).padStart(3, "0")}</p>
                  <h4>
                    <Link href={`/decks/${deck.id}`}>{deck.name}</Link>
                  </h4>
                  <p>
                    {deck.cardCount} cards · {deck.totalMisses} recorded misses
                  </p>
                  <div>
                    {Object.entries(deck.questionTypes)
                      .filter(([, count]) => count)
                      .map(([type, count]) => (
                        <span key={type}>
