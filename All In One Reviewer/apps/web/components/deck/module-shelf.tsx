...
export function ModuleShelf({ modules, selected, onChange }: Props) {
  if (!modules.length)
    return (
      <p className="quiet">
        Your uploaded modules will appear here. Select one or more to build a deck.
      </p>
    );
  return (
    <ul className="module-shelf" aria-label="Uploaded modules">
      {modules.map((module) => {
          ...
          <li key={module.id}>
            <label>
              <input
                ...
                onChange={() =>
                  onChange(
                    checked ? selected.filter((id) => id !== module.id) : [...selected, module.id],
                  )
                }
              />
              ...
              <span>
                ...
                <small>
                  {module.duplicate
                    ...
                    : module.extractionStatus === "pending"
                      ? "Ready for generation"
                      : module.extractionStatus}
                </small>
              </span>
