...
export function ModuleDropzone({ disabled, onFiles }: Props) {
  ...
  const accept = (files: FileList | File[]) => {
    ...
    const accepted = Array.from(files).filter((file) => {
      if (!/\.(pdf|pptx)$/i.test(file.name)) { rejections.push({ name: file.name, reason: "type" }); return false; }
      if (file.size > MAX_BYTES) { rejections.push({ name: file.name, reason: "size" }); return false; }
      return true;
    });
  ...
  return <div className={`dropzone ${dragging ? "dropzone-active" : ""}`} onDragOver={(event) => { event.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)} onDrop={drop}>
    ...
    {rejected.length > 0 && <ul className="dropzone-rejects" aria-live="polite">{rejected.map((file) => <li key={`${file.reason}-${file.name}`}>{file.name} — {file.reason === "size" ? "over 20 MB" : "not a PDF or PPTX"}</li>)}</ul>}
  </div>;
}
