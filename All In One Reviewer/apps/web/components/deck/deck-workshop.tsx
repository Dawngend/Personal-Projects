...
export function DeckWorkshop() {
  ...
  const upload = useMutation({
    mutationFn: api.uploadModules,
    onSuccess: (newModules) => {
      queryClient.invalidateQueries({ queryKey: ["modules"] });
      setSelectedModuleIds((current) => [
        ...new Set([...current, ...newModules.map((module) => module.id)]),
      ]);
    },
  });
  ...
  return (
    <main className={styles.page}>
      ...
      <div className={styles.intro}>
        ...
        <p>
          Andy builds a reviewer only from the modules you choose, then reports the generation path
          as it actually happens.
        </p>
      </div>
      <nav className={styles.steps} aria-label="Deck generation stages">
        ...
        <form className={styles.form} onSubmit={submit}>
          <section>
            <div className={styles.sectionHead}>
              ...
              <p>
                Upload course PDFs or slide decks. Existing uploads stay available for future decks.
              </p>
            </div>
            <ModuleDropzone
            ...
            />
            {upload.isPending && <p className="truthful-status">Uploading selected files…</p>}
            {upload.isError && <p className="form-error">{upload.error.message}</p>}
            <ModuleShelf
              modules={modules.data ?? []}
          ...
          <section>
            <div className={styles.sectionHead}>
              <p className="eyebrow">02 · Deck brief</p>
              <h2>Frame the review</h2>
              <p>Give the material a useful name, subject, and deliberate amount of practice.</p>
            </div>
            <DeckBriefForm register={form.register} errors={errors} />
          </section>
          <section>
            <div className={styles.sectionHead}>
              <p className="eyebrow">03 · Question style</p>
              <h2>Set the recall mode</h2>
              <p>
                Mixed is the default: exact choices, full enumerations, and final-answer problems.
              </p>
            </div>
            <QuestionStyleField register={form.register} />
          </section>
          <section className={styles.generate}>
            <div>
              <p className="eyebrow">04 · Generate</p>
              <h2>Make this deck</h2>
              <p>
                {selectedModuleIds.length
                  ? `${selectedModuleIds.length} selected module${selectedModuleIds.length === 1 ? "" : "s"} will be sent to the generation worker.`
            ...
            <button
              ...
              type="submit"
              disabled={generation.isPending || job?.status === "running"}
            >
          ...
          {job?.status === "complete" && job.deckId && (
            <p className={styles.complete}>
              Deck ready. <Link href={`/decks/${job.deckId}`}>Open this deck</Link> to begin
              studying.
            </p>
          )}
        </form>
